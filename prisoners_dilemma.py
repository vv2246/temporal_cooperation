...
class Agent():
    def __init__(self, idx, p_coop, T, modulate= True, timescale = 100,phase=0,amplitude = 0.35 ):
        ...
        if modulate:
            # amplitude = ampl#np.random.rand()#*0.1#0.1 # amplitude
        ...
        # strategy: cooperate with probability p
        self.idx = idx
        self.payoff_mat = np.array([[3,0],[5,1]])
        
    def decision(self , t, cp = True):
        r = np.random.rand()
        ### 1 means cooperating
        if self.p_coop[t] > r:
            res = 1
    ...
    def calc_payoff(self, graph , agents ):
        ...
        current_pay = 0
        # print(self.idx ,"has ", graph.degree(self.idx), "nbrs", list(graph.neighbors(self.idx)))
        for nbr in graph.neighbors(self.idx):
            ...
            current_pay += pay
            # print(current_pay,end= ",")
        # print("")
        self.payoff.append(current_pay)
        self.cumm_payoff.append(current_pay + sum(self.payoff))
            
    def print_stats(self, ax = None,plot=False):
        if self.payoff_calculated == False:
            assert "Need to calculate payoff first"
        if plot:
            # ax.plot(self.cumm_payoff, label= "p={:.3f}".format(self.p_coop_const))
